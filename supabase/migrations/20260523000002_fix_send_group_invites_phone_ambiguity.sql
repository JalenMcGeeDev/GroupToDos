-- Fix: column reference "phone" is ambiguous in send_group_invites.
-- RETURNS TABLE (phone TEXT, ...) creates an OUT param named "phone" that
-- clashes with group_invites.phone inside the function body, specifically in
-- ON CONFLICT (group_id, phone).
-- Fix: replace ON CONFLICT with an explicit SELECT + INSERT-or-UPDATE so
-- "phone" never appears as a bare conflict-target column. Use table alias
-- "gi" everywhere in WHERE clauses for unambiguous resolution.
...
BEGIN
  ...
  LOOP
    ...
    -- Upsert without ON CONFLICT column list to avoid OUT-param name clash.
    -- Check for an existing invite first (fully-qualified alias to prevent ambiguity).
    SELECT gi.id
      INTO v_invite_id
